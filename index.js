...
async function startHisoka() {
    const hisoka = hisokaConnect({
        ...
        browser: ['Zee Multi Device','Safari','1.0.0'],
        auth: state
    })
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    ...
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Keluar Dari Grup ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
